...
class SlackClient:
    ...
    def send_message(self, channel, text):
        ...
        Returns a SlackResponse object, can be converted to JSON via json.dumps(response['message'])
        """
    ...
    def delete_message(self, channel, message_id):
        ...
        Returns a SlackResponse object, can be resolved to True/False via response['ok']
        """
...
class SlackAdminClient(SlackClient):
    def get_users(self, limit=None):
        """ Get a list of users in the workspace

        Parameters:
        limit (Integer) <optional>: How many users you would like returned.

        OAuth Scopes Required
        ---
        users:read

        Returns a SlackResponse object, that can be converted to json via json.dumps(response['members'])
        """
        try:
